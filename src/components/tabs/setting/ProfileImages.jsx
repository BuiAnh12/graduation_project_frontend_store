...
const ProfileImages = ({ store }) => {
  const [avatarPreview, setAvatarPreview] = useState(
    store?.avatar?.url || "/assets/shop_logo.png"
  );
  const [coverPreview, setCoverPreview] = useState(
    store?.cover?.url || "/assets/default_cover.jpg"
  );
  ...
  const handleImageChange = async (e, type) => {
    ...
    try {
      ...
      await updateStoreInformation({
        storeId: store._id,
        updates: { [type]: newImage },
      });

      // Set preview
      type === "avatar"
        ? setAvatarPreview(newImage.url)
        : setCoverPreview(newImage.url);
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg p-4 flex flex-col items-center space-y-4">
        ...
        <label
          ...
          className="cursor-pointer bg-orange-500 text-white px-4 py-2 rounded-md"
        >
          Thay đổi ảnh
        </label>
      </div>
